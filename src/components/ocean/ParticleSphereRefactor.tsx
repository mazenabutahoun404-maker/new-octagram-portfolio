...
export default function ParticleSphereRefactor(__props: ParticleSphereRefactorProps) {
    ...
    useEffect(() => {
        ...
        if (particleShape === "sphere") {
            const sRadius = particleSize * 0.15
            const sphereGeometry = new SphereGeometry(sRadius, 8, 8)
        ...
        const animateCore = () => {
            ...
            if (pShape === "sphere" && particlesRef.current) {
                const matrix = new Matrix4()
                const instanceColorsAttr = particlesRef.current.instanceColor
                const colorArray = instanceColorsAttr ? (instanceColorsAttr.array as Float32Array) : null
                const len = baseParticlePositionsRef.current.length

                for (let i = 0; i < len; i++) {
                    const basePos = baseParticlePositionsRef.current[i]
                    const displacement = particleDisplacementsRef.current[i]
                    
                    // 3D Organic Wave Noise Deformation
                    const wave = Math.sin(timeSec * 1.5 + basePos.x * 2.2 + basePos.y * 3.1 + basePos.z * 1.8) * 0.035
                    const finalPos = new Vector3()
                    finalPos.copy(basePos)
                    finalPos.multiplyScalar(1 + wave)
                    finalPos.add(displacement)
                    
                    matrix.setPosition(finalPos.x, finalPos.y, finalPos.z)
                    particlesRef.current.setMatrixAt(i, matrix)

                    // Dynamic Iridescent Chromatic Spectrum
                    if (colorArray) {
                        const idx = i * 3
                        const hue = (timeSec * 0.08 + (basePos.y * 0.35 + basePos.x * 0.25 + 1) * 0.3) % 1
                        const sat = 0.95
                        const light = 0.6 + Math.sin(timeSec * 2 + i * 0.1) * 0.1
                        ...
                        colorArray[idx] = hueToRgb(hue + 1/3)
                        colorArray[idx + 1] = hueToRgb(hue)
                        colorArray[idx + 2] = hueToRgb(hue - 1/3)
                    }
                }
                particlesRef.current.instanceMatrix.needsUpdate = true
                if (instanceColorsAttr) instanceColorsAttr.needsUpdate = true
            ...
            const needsContinue =
                ...
                hasVelocity ||
                hasLerpDelta ||
                hasCursorInteraction) && isVisibleRef.current
